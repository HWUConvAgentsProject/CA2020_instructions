...
parser.add_argument('-rasa_model', type=str, default="rasa_tutorial/core/dstc_chatbot/")
...
class RASABot(Bot):
    def __init__(self, **kwargs):
        ...
        super(RASABot, self).__init__(bot_name=BOT_NAME)
        self.rasa_agent = kwargs.get("rasa_agent")

    def _call_rasa_agent(self, message):
        # Support function to handle asynchronous request to RASA agents
        # We use the AsyncIO library to wait until the RASA engine is able to serve our request
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        response = loop.run_until_complete(self.rasa_agent.handle_text(message))
        if response:
            # 'response' is a list containing a dictionary: 'recipient_id' and 'text'
    ...
    def post(self):
        ...
        resp = self._call_rasa_agent(user_utterance)
        self.response.result = resp
...
if __name__ == "__main__":
    ...
    # We load the RASA agent from the model file saved in args.rasa_model path
    rasa_agent = Agent.load(args.rasa_model)
    api.add_resource(RASABot, "/", resource_class_kwargs={
        "rasa_agent": rasa_agent
    })
